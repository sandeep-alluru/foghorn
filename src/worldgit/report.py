"""Rich terminal, JSON, and Markdown output formatters for worldgit."""
...
from worldgit.staleness import DiffResult
...
def print_diff(
    ...
    con.print(
        Panel(
            f"[bold]worldgit diff[/bold]  [dim]{a_id}[/dim] → [dim]{b_id}[/dim]",
            expand=False,
        )
    )
...
def to_markdown(alerts: list[StalenessAlert]) -> str:
    ...
    lines += [
        ...
        "</details>",
        "",
        "*Generated by [worldgit](https://github.com/sandeep-alluru/worldgit)*",
    ]
    return "\n".join(lines)
