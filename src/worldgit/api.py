"""FastAPI REST wrapper for worldgit.

Start:   uvicorn worldgit.api:app --reload
Install: pip install "worldgit[api]"
"""
...
try:
    ...
    raise ImportError("API server requires: pip install 'worldgit[api]'") from exc

from worldgit import __version__
...
app = FastAPI(
    ...
    license_info={
        "name": "MIT",
        "url": "https://github.com/sandeep-alluru/worldgit/blob/main/LICENSE",
    },
)
...
class FactRequest(BaseModel):
    ...
    db: str = Field(".worldgit/world.db")
...
@app.get("/stale")
async def stale(db: str = ".worldgit/world.db") -> Any:
...
@app.get("/log")
async def log(db: str = ".worldgit/world.db") -> Any:
